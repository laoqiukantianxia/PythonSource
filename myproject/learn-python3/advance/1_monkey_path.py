...
class ThrottledSocket(object):
    ...
    def recv(self, *args):
        ...
        if self._debug:
            ...
            print
            "Start: %s, End: %s, Expected end: %s" % (start, end,
                                                      expected_end)
            print
            "Actual: received %s bytes in %s seconds: rate=%sKB/s" % (
        ...
        if expected_end > end:  # only sleep if recv was quicker than expected
            ...
            if self._debug:
                print
                "  Sleeping % s seconds" % (expected_end - end)
        ...
        if self._debug:
            ...
            if duration == 0:
                ...
                rate = len(buf) / duration / 1024
            print
            "Effective: received %s bytes in %s seconds: rate=%sKB/s" % (
                len(buf), duration, rate)
        return buf
...
if __name__ == '__main__':
    ...
    patch()
    h = http.client.HTTPConnection("www.python.org")
    h.connect()
    ...
    print
    "Response %s bytes in %s sec (%sKB/s)" % (len(data), end - start,
