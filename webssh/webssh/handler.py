...
FILTER_COMMAND = {'ubuntu': {'ls': 0, 'ps': 0}}
...
class IndexHandler(MixinHandler, tornado.web.RequestHandler):
    ...
    def ssh_connect(self, args):
        ...
        # 匹配用户受限的命令
        filter_command = None
        if args[2] in FILTER_COMMAND.keys():
            logging.info('[{}] match filter command:{}'.format(args[2], FILTER_COMMAND[args[2]]))
            filter_command = FILTER_COMMAND[args[2]]
        worker = Worker(self.loop, ssh, chan, dst_addr, filter_command)
    ...
    def post(self):
        ...
        logging.info('post handler over.')
