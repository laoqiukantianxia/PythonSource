...
class Worker(object):
    def __init__(self, loop, ssh, chan, dst_addr, filter_command):
        self.loop = loop
        self.ssh = ssh
        self.chan = chan
    ...
    def match_command(self, data):

        if not self.filter_command or data == ' ':
            return False
        logging.debug('filter command:{}'.format(self.filter_command))

        for command in self.filter_command.keys():
            if self.filter_command[command] >= len(command):
                if data == '\r\n' or data == '\r':
                    logging.info('the command [{}] is not allow!'.format(command))
                    return True
                elif data == '\x7f': # 删除键
                    self.filter_command[command] -= 1
                #elif data == ';':
                #    pass
                else:
                    self.filter_command[command] += 1
            elif command.find(data) == self.filter_command[command]:
                logging.info('{} match {}'.format(data, command))
                self.filter_command[command] += 1
            elif data == '\x7f':  # 删除键
                if self.filter_command[command]:
                    self.filter_command[command] -= 1
            else :
                self.filter_command[command] = 0
